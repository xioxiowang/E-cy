...
# from models.EPSA import EPSABlock
from models.DAN import DAN, PAM_Module, CAM_Module
...
class C3_GC(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.GC = GlobalContext(c2)
        self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))

    def forward(self, x):
        return self.GC(self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1)))

class C3_EffectiveSE(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.EffectiveSE = EffectiveSEModule(c2)
        self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))

    def forward(self, x):
        return self.EffectiveSE(self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1)))

#尝试在C3中添加BAM注意力机制
class C3_BAM(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.BAM = BAMBlock(c2)
        self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))

    def forward(self, x):
        return self.BAM(self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1)))

class C3_DAN(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
    ...
    def forward(self, x):
        x1 = self.PAM(self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1)))
        x2 = self.CAM(self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1)))
        return x1 + x2
...
class C3_PSA(nn.Module):
    ...
    def forward(self, x):
        return self.PSA(self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1)))

class C3_EPSA(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
    ...
    def forward(self, x):
        return self.EPSA(self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1)))

class C3_SPSA(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.SequentialPolarizedSelfAttention = SequentialPolarizedSelfAttention(c2)
        self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))

    def forward(self, x):
        return self.SequentialPolarizedSelfAttention(self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1)))

class C3_MobileViTAttention(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.MobileViTAttention = MobileViTAttention(c2)
        self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))

    def forward(self, x):
        return self.MobileViTAttention(self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1)))
...
class C3_Biformer(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(2 * c_, c2, 1)  # optional act=FReLU(c2)
        self.BiLevelRoutingAttention = BiLevelRoutingAttention(c2)
        self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))

    def forward(self, x):
        return self.BiLevelRoutingAttention(self.cv3(torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1)))
...
#
#         self.avg_pool = nn.AdaptiveAvgPool2d(1)
#         self.fc = nn.Conv2d(4 * c, 4 * c, kernel_size=1) # 注意输出通道数的设置
#         self.si = nn.Sigmoid()
#         self.cv = nn.Conv2d(4 * c, c, kernel_size=1)
#
#     def forward(self, x):
#         x = x.to(next(self.parameters()).device)
#         b, c, _, _ = x.size()
#         y = self.avg_pool(x).view(b, c, 1, 1)
#         weights =self.fc(y) # 使用 Sigmoid 激活函数
#         y = x * y.expand_as(x)  # 使用权重调整输入特征
#         return self.cv(y)
...
class MDown(nn.Module):
    def __init__(self, c1, c2):
        super().__init__()
        # self.c = c2 // 2
        self.cv = GSConv(c1, c1, 3, 1)#把填充去掉试一试
...
# 修改1(先减少通道数在进行MAXPOOL，也可以试试先MAXPOOL再减少通道数)
class SPPF1(nn.Module):
    ...
    def __init__(self, c1, c2, k=5):  # equivalent to SPP(k=(5, 9, 13))
        ...
        c_1 = c_ // 2
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv11 = Conv(c_, c_1, 1, 1)
        self.cv2 = Conv(c_ * 4, c2, 1, 1)
        self.Conv1 = GSConv(c_, c_, 3, 1)
        self.Conv2 = GSConv(c_1, c_1, 3, 1)
    ...
    def forward(self, x):
        ...
        with warnings.catch_warnings():
            ...
            y_3 = self.Conv2(y3)
            x = self.Conv1(x)
            y = torch.cat((y1, y2, y3, y_1, y_2, y_3),1)
...
class SPPF4(nn.Module):
    # Spatial Pyramid Pooling - Fast (SPPF) layer for YOLOv5 by Glenn Jocher
    def __init__(self, c1, c2, k=5):  # equivalent to SPP(k=(5, 9, 13))
        super().__init__()
        c_ = c1 // 2  # hidden channels
        self.cv1 = GSConv(c1, c_, 1, 1)
        self.cv2 = GSConv(c1, c2, 1, 1)
        self.cv3 = GSConv(c_ * 6, c_, 1, 1)
        self.Conv1 = GSConv(c_, c_, 3, 1)
        self.Conv2 = GSConv(c1, c1, 3, 1)
        # self.Conv2 = GSConv(c_ * 6, c_ * 6, 3, 1)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
    ...
    def forward(self, x):
        ...
        with warnings.catch_warnings():
            ...
            x = self.Conv1(x)
            y = torch.cat((y1, y2, y3, y_1, y_2, y_3),1)
            y = self.Conv1(self.cv3(y))
            return self.cv2(self.Conv2(torch.cat((x, y), 1)))

class SPPF5(nn.Module):
    # Spatial Pyramid Pooling - Fast (SPPF) layer for YOLOv5 by Glenn Jocher
    def __init__(self, c1, c2, k=5):  # equivalent to SPP(k=(5, 9, 13))
        super().__init__()
        c_ = c1 // 2  # hidden channels
        ...
        self.cv3 = Conv(c_ * 7, c_, 1, 1)
        self.Conv1 = GSConv(c_, c_, 3, 1)
        self.Conv2 = GSConv(c_, c_, 3, 1)
        self.Conv3 = GSConv(c_, c_, 3, 1)
        self.Conv4 = GSConv(c_, c_, 3, 1)
        self.Conv5 = GSConv(c_, c_, 3, 1)
        self.Conv6 = GSConv(c_, c_, 3, 1)
        self.Conv7 = GSConv(c1, c1 , 3, 1)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
    ...
    def forward(self, x):
        ...
        with warnings.catch_warnings():
            ...
            y_3 = self.Conv3(y3)
            x = self.Conv4(x)
            y = self.cv3(torch.cat((y1, y2, y3, y_1, y_2, y_3, x),1))
            x = self.Conv5(x)
            y = self.Conv6(y)
            return self.cv2(self.Conv7(torch.cat((x, y), 1)))
...
class EMSConvP(nn.Module):
    ...
    def __init__(self, c):
        ...
        min_ch = c // self.groups
        assert min_ch >= 16, f'channel must Greater than {16 * self.groups}, but {c}'

        self.convs = nn.ModuleList([])
        for ks in kernels:
            self.convs.append(Conv(c1=min_ch, c2=min_ch, k=ks))
        self.avg_pool = torch.nn.AvgPool2d(kernel_size=2, stride=2)
        self.conv_1x1 = Conv(c, c, k=1)
...
class C3_EMSC(C3):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(Bottleneck_EMSC(c_, c_, shortcut, g, k=((1, 1), (3, 3))) for _ in range(n)))
...
#把这几个卷积换成GSConv参数量应该还会降低
class PSAModule(nn.Module):
# def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1,  act=True):
#     super().__init__()
#         c_ = c2 // 2
#         self.cv1 = Conv(c1, c_, k, s, p, g, d,  act=True)
#         self.cv2 = Conv(c_, c_, 5, 1, p, c_, d, act=True)
    def __init__(self, inplans, planes, conv_kernels=[3, 5, 7, 9], stride=1, conv_groups=[1, 4, 8, 16]):
        super(PSAModule, self).__init__()
        # self.conv_1 = GSConv(inplans, planes//4, k=conv_kernels[0], s=stride, p=conv_kernels[0]//2,
        #                      g=conv_groups[0])
        # self.conv_2 = GSConv(inplans, planes//4, k=conv_kernels[1], s=stride, p=conv_kernels[1]//2,
        #                      g=conv_groups[1])
        # self.conv_3 = GSConv(inplans, planes//4, k=conv_kernels[2], s=stride, p=conv_kernels[2]//2,
        #                      g=conv_groups[2])
        # self.conv_4 = GSConv(inplans, planes//4, k=conv_kernels[3], s=stride, p=conv_kernels[3]//2,
        #                      g=conv_groups[3])
...
class EPSABlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, norm_layer=None, conv_kernels=[3, 5, 7, 9],
        ...
        self.stride = stride
        self.Conv1 = GSConv(inplanes, planes, 3, 1)
...
class SPPF2(nn.Module):
    ...
    def __init__(self, c1, c2, k=5):  # equivalent to SPP(k=(5, 9, 13))
        ...
        self.Conv1 = GSConv(c_, c_, 3, 1)
        self.Conv2 = GSConv(c_1, c_1, 3, 1)
        self.Conv3 = GSConv(2*c_, 2*c_, 3, 1)
        self.Conv4 = GSConv(3*c_, 3*c_, 3, 1)
        # self.Conv3 = GSConv(c_2, c_2, 3, 1)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
    ...
    def forward(self, x):
        ...
        with warnings.catch_warnings():
            ...
            y1 = self.m(x)
            y2 = self.m(y1)
            y3 = self.m(self.cv11(y2))
            y_3 = torch.cat((self.Conv2(y3), y3), 1)
            y_2 = self.Conv3(torch.cat((y_3, y2), 1))
            y = self.Conv4(torch.cat((y_2, y1), 1))
...
class SPPF2I(nn.Module):
    # Spatial Pyramid Pooling - Fast (SPPF) layer for YOLOv5 by Glenn Jocher
    def __init__(self, c1, c2, k=5):  # equivalent to SPP(k=(5, 9, 13))
        super().__init__()
        c_ = c1 // 2  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(c_ * 3, c_, 1, 1)
        self.cv2 = Conv(c1, c2, 1, 1)
        self.Conv1 = GSConv(c_, c_, 3, 1)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
    ...
    def forward(self, x):
        ...
        with warnings.catch_warnings():
            ...
            y2 = self.m(y1)
            y3 = self.m(y2)
            y_3 = torch.cat((self.Conv1(y3), y3), 1)
            y_2 = self.Conv1(self.cv3(torch.cat((y_3, y2), 1)))
            y = self.Conv1(self.cv1(torch.cat((y_2, y1), 1)))
            x = self.Conv1(x)
            return self.cv2(torch.cat((x, y), 1))


class SPPF3(nn.Module):
    # Spatial Pyramid Pooling - Fast (SPPF) layer for YOLOv5 by Glenn Jocher
    def __init__(self, c1, c2, k=5):  # equivalent to SPP(k=(5, 9, 13))
        super().__init__()
        c_ = c1 // 2  # hidden channels
        ...
        self.cv2 = Conv(c_ * 4, c2, 1, 1)
        self.Conv1 = GSConv(c_, c_, 3, 1)
        self.Conv2 = GSConv(3*c_, 3*c_, 3, 1)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
...
class Detections:
    ...
    def save(self, labels=True, save_dir='runs/detect/voc-dyhead+epsa+sppf5', exist_ok=False):
        save_dir = increment_path(save_dir, exist_ok, mkdir=True)  # increment save_dir
        self._run(save=True, labels=labels, save_dir=save_dir)  # save results

    def crop(self, save=True, save_dir='runs/detect/voc-dyhead+epsa+sppf5', exist_ok=False):
...
class SCE(nn.Module):
    def __init__(self, c1):
        ...
        self.conv = Conv(c1[0] + c1[1], c1[0] + c1[1], k=3)
        self.conv1 = Conv(c1[0], c1[0], k=3)
        self.conv2 = Conv(c1[1], c1[1], k=3)

    def forward(self, x):
        x_p1, x_p2 = x
        x_p1 = self.conv1(x_p1)#添加方法二
        x_p2 = self.conv2(x_p2)#添加方法二，这样的话参数量肯定会暴增
...
class DPE(nn.Module):
    def __init__(self, c1, c2):
        ...
        self.conv0 = Conv(c1[0], c1[0], k=3)
        self.conv1 = Conv(c1[1], c1[1], k=3)
        self.conv2 = Conv(c1[2], c1[2], k=3)

    def forward(self, x):
        x[0] = self.conv0(x[0])#添加方法二
        x[1] = self.conv1(x[1])#添加方法二
        x[2] = self.conv2(x[2])#添加方法二
...
class C3_BiLevelRoutingAttention(C3):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
...
class SeparableConv(nn.Module):
    # Separable convolution with args(ch_in, ch_out, kernel, stride, padding, dilation, activation)
    default_act = nn.SiLU()  # default activation
